...
export default function AgentSettings() {
  ...
  useEffect(() => {
    ...
    const loadDoc = async () => {
      ...
      try {
        ...
        switch (activeDoc) {
          case 'agent': res = await getAgentDocument(agentId); break
          case 'identity': res = await getIdentityDocument(agentId); break
          case 'heartbeat': res = await getHeartbeatDocument(agentId); break
        }
        const content = res.data?.content || ''
        setDocContent(content)
        setDocOriginal(content)
      } catch {
        addToast('error', `Failed to load ${activeDoc.toUpperCase()}.md`)
        setDocContent('')
        setDocOriginal('')
      } finally {
        setDocLoading(false)
  ...
  const handleSaveDoc = async () => {
    ...
    try {
      switch (activeDoc) {
        case 'agent': await updateAgentDocument(agentId, docContent); break
        case 'identity': await updateIdentityDocument(agentId, docContent); break
        case 'heartbeat': await updateHeartbeatDocument(agentId, docContent); break
      }
      setDocOriginal(docContent)
      addToast('success', `${activeDoc.toUpperCase()}.md saved`)
    } catch (err: unknown) {
      addToast('error', `Failed to save ${activeDoc.toUpperCase()}.md`, getErrorMessage(err))
    } finally {
      setDocSaving(false)
    }
  }
  ...
  return (
    <>
      ...
      <div className="main-body" style={{ display: 'flex', padding: 0 }}>
        {/* Desktop sidebar nav */}
        <div className="hidden md:block" style={{ width: '200px', borderRight: '1px solid var(--border)', padding: '24px 16px', flexShrink: 0 }}>
        ...
        <div className="flex-1 overflow-auto" style={{ padding: '24px' }}>
          ...
          {activeTab === 'documents' && (
            <div>
              ...
              {/* Editor */}
              {docLoading ? (
                <p style={{ color: 'var(--text-tertiary)' }}>Loading document...</p>
              ) : (
          ...
          {activeTab === 'danger' && (
            <div style={{ maxWidth: '600px' }}>
              ...
              <div style={{ border: '1px solid #ef4444', borderRadius: '8px', padding: '1.5rem' }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '13px', marginBottom: '1rem', cursor: 'pointer', color: 'var(--text-secondary)' }}>
