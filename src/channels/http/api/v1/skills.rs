...
pub fn skills() -> Router<HTTPState> {
    Router::new()
        .route("/", get(list_skills).post(create_skill))
        .route("/{slug}", get(get_skill).put(update_skill).delete(delete_skill))
        .route("/{slug}/resources", get(list_resources))
        .route("/{slug}/resources/{path}", get(read_resource))
}
...
async fn list_resources(
    ...
    let resources = match manager.list_resources(&slug) {
        Ok(resources) => resources,
        Err(_) => return err_response(StatusCode::INTERNAL_SERVER_ERROR, "Failed to list resources".into()),
    };
    let paths: Vec<String> = resources.into_iter().map(|p| p.to_string_lossy().to_string()).collect();

    api_response(StatusCode::OK, paths)
}
...
async fn read_resource(
    ...
    let content = match manager.read_resource(&slug, &path) {
        Ok(content) => content,
        Err(_) => return err_response(StatusCode::INTERNAL_SERVER_ERROR, "Failed to read resource".into()),
    };

    match content {
        Some(content) => api_response(StatusCode::OK, content),
        None => err_response(StatusCode::NOT_FOUND, "Resource not found".into()),
    }
}
