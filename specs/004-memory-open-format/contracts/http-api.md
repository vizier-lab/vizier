...
| `DELETE /bundles/{bundle}` | Deletes a bundle (its `index.md`/`log.md`). Without `?force=true`, `409`-class error if it still holds any concept document — same empty-only behavior as the `memory_delete_bundle` agent tool. With `?force=true` (WebUI operator path only, behind a confirmation modal — never sent by the agent tool), every remaining concept document is deleted too. |
